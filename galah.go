...
func (app *App) handleRequest(w http.ResponseWriter, r *http.Request, serverAddr string) {
	...
	if app.Verbose {
		log.Printf("Received a request for %q from %s", r.URL.String(), r.RemoteAddr) // change this to work for any LLM
	}
    ...
    //log workflow for data collection 
...
func (app *App) generateAndCacheResponse(r *http.Request, port string) ([]byte, error) {
	responseString, err := GenerateOpenAIResponse(app.Config, r) //change var name
	if err != nil {
		log.Print(err)
		return nil, err
	}
